          ...
          this.venueUserService.bookEvent(this.booking).subscribe(
            (data)=>{ this.check=data;
              if(this.check == 1){
                ...
                Swal.fire("Ohh No","It seems that this policy  is already taken from this Issuer"+
                 " Please try to choose another date  ","error")
              }
              console.log("check is "+this.check)
              
            },(error)=>{
              Swal.fire("Sorry","There is an error while Booking this Policy","error")
            }
          )
        }})
